  ...
  Sequenza Eventi &
   ...
   \begin{enumerate}
    \item il sistema controlla che l'indirizzo non sia già presente
     \item IF indirizzo email non è presente e la password è valida
     \begin{enumerate}
...
Post-Condizioni: & Il cliente è registrato nel sistema \\
  ...
  \item Se nel punto 6 l'indirizzo email non è valido viene mostrato un messaggio di errore all'utente e gli si chiede di inserire un nuovo indirizzo email
\item Se nel punto 6.2 l'indirizzo email è già presente nel sistema si mostra un messaggio di errore e si chiede all'utente di usare credenziali diverse
\item Se nel punto 6.2 la password risulta non valida viene mostrato un messaggio di errore all'utente e gli viene chiesto di inserire una nuova password
  \end{itemize}\\
\hline
\end{tabular} \\
...
\item L'organizzatore clicca sul tasto assegna revisore
                   ...
                   \item IF viene rispettata la data di scadenza
                   \begin{enumerate}
...
\hline
Sequenza degli eventi alternativa & \begin{itemize}
\item Nel punto 5 se la data di scadenza della revisione non viene rispetata, l'articolo viene automaticamente rifiutato
\item Nel punto 5.1 se la VerificaConflittiInterrese risulta negativo, l'organizzatore deve selezionare altri revisori
\item Nel punto 5.1.2 se InserimentoEsitoRevisione ha esito negativo, l'articolo automaticamente rifiutato
\end{itemize}\\
\hline
\end{tabular}
...
\hline
Sequenza degli eventi alternativa & \begin{itemize}
  \item Nel punto 3 se la verifica non va buon fine l'articolo non viene assegnato ai revisori
\end{itemize} \\
\hline
\end{tabular}
